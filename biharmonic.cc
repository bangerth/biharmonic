...
#define delete_files_at_start_flg 0
// if true, then the program will delete a bunch of files at startup to ensure
//          that "terminate" and output files don't already exist
// else it assumes that the calling program deleted them already.
...
std::string dii_biharmonic_prm_filename_st = dii_biharmonic_prm_filename_base_d;
std::string dii_visualization_dir_name_st = dii_visualization_dir_name_base_d;
std::string file_prepend_st;  // prepended to all above strings to produce unique i/o filenames (did this to try to keep different instances of the solver from colliding with each other)
int index_solver_base;  // added to the solver index when generating individual frequency output files
...
/** void generate_simulation_end_file(std::string &string_for_file)
	  generates the simulation_end_filename_d file and places the passed string inside of it
*/
void generate_simulation_end_file(std::string &string_for_file) {
	std::ofstream simulation_end_stream;
	simulation_end_stream = std::ofstream(instance_folder + "/" + dii_simulation_end_filename_st);
	simulation_end_stream << string_for_file;
}
...
namespace MembraneOscillation
{
	...
	{
		// First read parameter values from the input file 
		prm.parse_input(instance_folder + "/" + dii_biharmonic_prm_filename_st);
	...
	struct OutputData
	{
		...
		int index;
		// index of the frequencies being generated
		// < 0 if not set
		// if the index has been set, it will be assumed that the individual output 
		// file for this frequency has been written
	};
	...
	// placing the word "STOP" into the file "termination_signal.txt" in the
	// current directory.
	//
	...
	bool check_for_termination_signal()
	{
		...
		// output file.)
		std::ifstream in(instance_folder + "/" + dii_termination_signal_filename_st);
		if (!in)
		{
		...
		//if (line == "STOP")
		{
			...
			in.close();
			//std::remove ((instance_folder + "/" + dii_termination_signal_filename_st).c_str());
			...
			generate_simulation_end_file(std::string("terminated upon request"));  // indicate that the simulation has ended
			std::exit(1);
		...
		BiharmonicProblem<dim>::output_results()
	{
		...
		std::string file_name = instance_folder + "/" + dii_visualization_dir_name_st + "/solution-" +
	...
	void solve_one_frequency(const double omega)
	{
		...
		// working on the frequency this task corresponds to.
		  //printf("solve_one_frequency freq = %lg\n", omega / 6.28);
		...
		double highest_w = -1;
		int    i_hightest = -1;
		for (std::size_t i = 0; i < MaterialParameters::frequencies.size(); ++i) {
			auto it = results.find(MaterialParameters::frequencies[i]);
			if (it == results.end()) {
				break;
			}
			else {
				highest_w = MaterialParameters::frequencies[i];
				i_hightest = i;
			}
		}
		buffer << "   Highest sequential freq complted = " << highest_w / (2 * numbers::PI) << " (Hz) at index = " << i_hightest;
		buffer << std::endl;
		...
		for (auto result : results)
		{
			...
			buffer << st.str() << std::endl;


			// has the index for this frequency result been set?
			if (result.second.index < 0)
			{
				// find the index for this frequency
				for (std::size_t i = 0; i < MaterialParameters::frequencies.size(); ++i) {
					if (MaterialParameters::frequencies[i] == result.first)
					{
						result.second.index = i;
						break;
					}
				}
				...
				if (result.second.index >= 0)
				{
					// write the results for this frequency to an indivudla file.
					st2.str("");
					st2 << instance_folder << "\\";
					st2 << file_prepend_st << "I " << (result.second.index + index_solver_base) << ".txt";
					std::ofstream individual_frequency_response(st2.str());
					individual_frequency_response << st.str();
				}
			}

		}

		std::ofstream frequency_response(instance_folder + "/" + dii_frequency_response_filename_st);
...
int main(int argc, char *argv[])
{
	...
	if (argc >= 3)
	{
		...
		{
			file_prepend_st = argv[2];

			dii_termination_signal_filename_st = file_prepend_st + dii_termination_signal_filename_st;
			dii_output_log_filename_st = file_prepend_st + dii_output_log_filename_st;
			dii_frequency_response_filename_st = file_prepend_st + dii_frequency_response_filename_st;
			dii_simulation_end_filename_st = file_prepend_st + dii_simulation_end_filename_st;
			dii_biharmonic_prm_filename_st = file_prepend_st + dii_biharmonic_prm_filename_st;
			dii_visualization_dir_name_st = file_prepend_st + dii_visualization_dir_name_st;
		}
	}
	if (argc >= 4)
	{
	...
	logger = std::ofstream(instance_folder + "/" + dii_output_log_filename_st);
	...
	try
	{
		...
		// *********** testing code to see if we can effect trhead execution 
	  ...
	  // first time.
		std::remove((instance_folder + "/" + dii_frequency_response_filename_st).c_str());
		std::remove((instance_folder + "/" + dii_simulation_end_filename_st).c_str());

		//printf("************* starting to print directory contents *************\r\n");
		std::string st, st2;
		st = instance_folder;
		st += "\\I *.txt";
		std::wstring instance_folder_w;
		instance_folder_w.assign(st.begin(), st.end());
		const wchar_t* wcs = instance_folder_w.c_str();
		long hFile;
		_wfinddata64_t fileinfo;
		hFile = _wfindfirst64(wcs, &fileinfo);
		while (hFile != -1)
		{
			//convert from wide char to narrow char array
			std::wstring st_w(fileinfo.name);
			st2.assign(st_w.begin(), st_w.end());
			st = instance_folder;
			st += "\\";
			st += st2;
			//printf(st.c_str()); printf("\r\n");
			std::remove(st.c_str());

			int ret = _wfindnext64(hFile, &fileinfo);
			if (ret != 0)
				hFile = -1;
		}
#endif

		//printf("************* finished to print directory contents *************\r\n");
			...
			(n_threads >= MaterialParameters::frequencies.size()))
		{
			...
			generate_simulation_end_file(std::string("success"));
		}
		else
	...
	catch (std::exception &exc)
	{
		...
		generate_simulation_end_file(std::string("error"));
