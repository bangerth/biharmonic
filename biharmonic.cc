...
namespace StepBiharmonic
{
    ...
    public:
       ...
       virtual void hessian_list (const std::vector<Point<dim> > &points, std::vector< SymmetricTensor< 2, dim > > &hessians,
           const unsigned int            component = 0) const
        {
    ...
    public:
        ...
        {
          return 4*std::pow(PI,4.0)*sin(PI*p[0])
              *sin(PI*p[1]);
        }
    };
    }
    ...
    /* The commented part below corresponds to the polynomial solution as the benchmark.*/
     /*
    template <int dim>
    double
    ...
    {
        return std::pow(p(0),2)*std::pow(p(1),2)*(1-p(0))*(1-p(0))*(1-p(1))*(1-p(1));
                                 ...
                                 const unsigned int) const
    {
        ...
        return_value[0] = 2*p(0)*p(1)*p(1)*(p(0) - 1)*(p(0) - 1)*(p(1) - 1)*(p(1) - 1) + p(0)*p(0)*p(1)*p(1)*(2*p(0)- 2)*(p(1) - 1)*(p(1) - 1) ;
        return_value[1] = 2*p(0)*p(0)*p(1)*(p(0) - 1)*(p(0) - 1)*(p(1) - 1)*(p(1) - 1) +p(0)*p(0)*p(1)*p(1)*(2*p(1) - 2)*(p(0) - 1)*(p(0) - 1) ;
  ...
  void
				 ...
				 const unsigned int) const
    {
	  ...
	  for (unsigned int d = 0 ; d < dim ; ++d)
		{
		  hessians[i][d][d]=
2*std::pow(p[d],2)*std::pow(p[(d+1)%dim],2)*(p[(d+1)%dim] - 1)*(p[(d+1)%dim] - 1) + 2*std::pow(p[(d+1)%dim],2)*(p[d] - 1)*(p[d] - 1)*(p[(d+1)%dim] - 1)*(p[(d+1)%dim] - 1)+ 4*p[d]*std::pow(p[(d+1)%dim],2)*(2*p[d] - 2)*(p[(d+1)%dim] - 1)*(p[(d+1)%dim] - 1);
		  hessians[i][d][(d+1)%dim]=2*p[d]*std::pow(p[(d+1)%dim],2)*(2*p[(d+1)%dim] - 2)*(p[d] - 1)*(p[d] - 1) + 2*std::pow(p[d],2)*p[(d+1)%dim]*(2*p[d] - 2)*(p[(d+1)%dim] - 1)*(p[(d+1)%dim] - 1) + std::pow(p[d],2)*std::pow(p[(d+1)%dim],2)*(2*p[d] - 2)*(2*p[(d+1)%dim] - 2) + 4*p[d]*p[(d+1)%dim]*(p[d] - 1)*(p[d] - 1)*(p[(d+1)%dim] - 1)*(p[(d+1)%dim] - 1);
                                      ...
                                      const unsigned int component) const
    {
        ...
        return (24*std::pow(p(0),4) - 48*std::pow(p(0),3) + 288*std::pow(p(0),2)*std::pow(p(1),2) - 288*std::pow(p(0),2)*p(1) + 72*std::pow(p(0),2) - 288*p(0)*std::pow(p(1),2) + 288*p(0)*p(1) - 48*p(0) + 24*std::pow(p(1),4) - 48*std::pow(p(1),3) + 72*std::pow(p(1),2) - 48*p(1) + 8);
   ...
   */
    ...
    Solution<dim>::value (const Point<dim> &p,
                              const unsigned int) const
    {
         return std::sin(PI*p(0))*std::sin(PI*p(0))*std::sin(PI*p(1))*std::sin(PI*p(1));
                                 ...
                                 const unsigned int) const
    {
        ...
        return_value[0] = 2*PI*std::cos(PI*p(0))*std::sin(PI*p(0))*std::sin(PI*p(1))*std::sin(PI*p(1)) ;
        return_value[1] = 2*PI*std::cos(PI*p(1))*std::sin(PI*p(0))*std::sin(PI*p(0))*std::sin(PI*p(1)) ;
        return return_value;
    }
  ...
  void
                 ...
                 const unsigned int) const
    {
      ...
      for (unsigned int d = 0 ; d < dim ; ++d)
        {
          hessians[i][d][d]=
        2*PI*PI*std::cos(PI*p[d])*std::cos(PI*p[d])*std::sin(PI*p[(d+1)%dim])*std::sin(PI*p[(d+1)%dim]) - 2*PI*PI*std::sin(PI*p[d])*std::sin(PI*p[d])*std::sin(PI*p[(d+1)%dim])*std::sin(PI*p[(d+1)%dim]);
                  hessians[i][d][(d+1)%dim]=4*PI*PI*std::cos(PI*p[d])*std::cos(PI*p[(d+1)%dim])*std::sin(PI*p[d])*std::sin(PI*p[(d+1)%dim]);
                  hessians[i][(d+1)%dim][d]= hessians[i][d][(d+1)%dim];
        }
        }
                                      ...
                                      const unsigned int component) const
    {
        ...
        return (8*std::pow(PI,4)*(8*std::sin(PI*p(0))*std::sin(PI*p(0))*std::sin(PI*p(1))*std::sin(PI*p(1)) - 3*std::sin(PI*p(0))*std::sin(PI*p(0)) - 3*std::sin(PI*p(1))*std::sin(PI*p(1)) + 1));
                                 ...
                                 typename MeshWorker::IntegrationInfo<dim> &info) const
{
    ...
    dealii::LocalIntegrators::Biharmonic::cell_matrix(dinfo.matrix(0,false).matrix, info.fe_values(0));
                                     ...
                                     typename MeshWorker::IntegrationInfo<dim> &info) const
{
    ...
    dealii::LocalIntegrators::Biharmonic::weak_boundary_matrix(dinfo.matrix(0,false).matrix, info.fe_values(0),
                                                               dealii::LocalIntegrators::Biharmonic::compute_penalty(dinfo, dinfo, deg, deg));
                                 ...
                                 typename MeshWorker::IntegrationInfo<dim> &info2) const
{
    ...
    dealii::LocalIntegrators::Biharmonic::ip_matrix(dinfo1.matrix(0,false).matrix, dinfo1.matrix(0,true).matrix,
                                                    dinfo2.matrix(0,true).matrix, dinfo2.matrix(0,false).matrix,
                                                    info1.fe_values(0), info2.fe_values(0),
                                                    dealii::LocalIntegrators::Biharmonic::compute_penalty(dinfo1, dinfo2, deg, deg));
...
{   const RightHandSide<dim>  right_hand_side;
    ...
    for (unsigned k=0; k<fe.n_quadrature_points; ++k)
    {
        ...
        {
            local_vector(i) +=(fe.shape_value(i,k) * right_hand_side.value (fe.quadrature_point (k)) * fe.JxW(k));
        }
    }
...
void RHSIntegrator<dim>::boundary(MeshWorker::DoFInfo<dim> &dinfo, typename MeshWorker::IntegrationInfo<dim> &info) const
{
    ...
    const double penalty=dealii::LocalIntegrators::Biharmonic::compute_penalty(dinfo, dinfo, deg, deg);
    for (unsigned k=0; k<fe.n_quadrature_points; ++k)
    {
        ...
        for (unsigned int i=0; i<fe.dofs_per_cell; ++i)
            local_vector(i) += (-contract3(n ,fe.shape_hessian(i,k), n) * (n * exact_gradients[k])+2.* penalty*(n * fe.shape_grad(i,k)) * (n * exact_gradients[k]) )* fe.JxW(k);
                                ...
                                typename MeshWorker::IntegrationInfo<dim> &info) const
{
    ...
    for (unsigned k=0; k<fe.n_quadrature_points; ++k)
    {
        ...
        //exact_hessians[k][0][1] -=DDuh[k][0][1]; exact_hessians is a symmetric tensor, so don't edit both
        ...
        for (unsigned int d=0; d<dim; ++d)
        {
            ...
            sum += (0*diff_grad*diff_grad + diff_hess1*diff_hess1 + diff_hess2*diff_hess2);
        }
                                ...
                                typename MeshWorker::IntegrationInfo<dim> &info2) const
{
    ...
    const unsigned int deg = fe.get_fe().tensor_degree();
    const double penalty=dealii::LocalIntegrators::Biharmonic::compute_penalty(dinfo1, dinfo2, deg, deg);
    ...
    for (unsigned k=0; k<fe.n_quadrature_points; ++k)
      {
        ...
        const double t = pow(h,2) * rhs_values[k]; //(4*pow(PI,4)*std::sin(x)*std::sin(y));//dinfo.cell->diameter() * trace(DDuh[k]);
...
Tensor<1,dim>  diff_1  = dealii::LocalIntegrators::Biharmonic::second_partial_n(n,info1.hessians[0][0][k]);
               diff_1 -= dealii::LocalIntegrators::Biharmonic::second_partial_n(n,info2.hessians[0][0][k]);
...
template <int dim>
void Estimator<dim>::face(MeshWorker::DoFInfo<dim>& dinfo1, MeshWorker::DoFInfo<dim>& dinfo2,
    typename MeshWorker::IntegrationInfo<dim>& info1,  typename MeshWorker::IntegrationInfo<dim>& info2)
{
...
void BiharmonicProblem<dim>::setup_system ()
{
    ...
    DoFTools::make_flux_sparsity_pattern(dof_handler, c_sparsity,constraints,true);
...
void BiharmonicProblem<dim>::assemble ()
  {
    ...
    auto cell_worker = [&](const Iterator &  cell,
      ...
      for (unsigned int point = 0; point < fe_v.n_quadrature_points; ++point)
		...
		// \int_Z \nu \nabla^2 u \cdot \nabla^2 v \, dx.
		  ...
		  nu
		  * scalar_product(fe_v.shape_hessian(i, point), fe_v.shape_hessian(j, point))
		  * JxW[point];                // dx
              }
    ...
    auto face_worker = [&](const Iterator &    cell,
      ...
      const std::vector<Tensor<1, dim>> &normals =
        fe_i.get_normal_vectors();

      // eta = 1/2 + 2C_2
      // gamma = eta/|e|

      double gamma = 1.0; // TODO: dealii::LocalIntegrators::Biharmonic::compute_penalty(dinfo1, dinfo2, deg, deg);

      {
	int degree = fe.tensor_degree();
	const unsigned int normal1 = GeometryInfo<dim>::unit_normal_direction[f];
	const unsigned int normal2 = GeometryInfo<dim>::unit_normal_direction[nf];
	const unsigned int deg1sq = degree*(degree+1);//(deg1 == 0) ? 1 : deg1 * (deg1+1);
	const unsigned int deg2sq = degree*(degree+1);//(deg2 == 0) ? 1 : deg2 * (deg2+1);
...
//                    -(trace(fe_i.average_hessian(i,qpoint)))
    ...
    auto boundary_worker = [&](const Iterator &    cell,
      ...
      {
	int degree = fe.tensor_degree();
	const unsigned int normal1 = GeometryInfo<dim>::unit_normal_direction[face_no];
	const unsigned int deg1sq = degree*(degree+1);//(deg1 == 0) ? 1 : deg1 * (deg1+1);
    ...
    ScratchData<dim> scratch_data(mapping, fe, n_gauss_points, update_values |
                                                 update_gradients |
				  update_hessians |
                                                 update_quadrature_points |
                                                 update_JxW_values,
...
void BiharmonicProblem<dim>::assemble_system ()
{
    ...
    UpdateFlags update_flags = update_values | update_quadrature_points | update_gradients | update_hessians;
...
template <int dim>
void
BiharmonicProblem<dim>::assemble_right_hand_side()
...
template <int dim>
void
BiharmonicProblem<dim>::error(const unsigned int cycle)
{
...
template <int dim>
void BiharmonicProblem<dim>::output_results (const unsigned int iteration) const
{
    ...
    VectorTools::project(mapping, dof_handler, constraints, QGauss<dim>(degree+1), exact_solution, exact);
    data_out.add_data_vector(exact, "exact");

    data_out.build_patches ();

    std::ofstream output_vtk ((std::string("output_") +
                               Utilities::int_to_string (iteration, 6) +
                               ".vtk").c_str ());
...
template <int dim>
double
BiharmonicProblem<dim>::estimate()
{
...
void BiharmonicProblem<dim>::run ()
{
  ...
  if (true)
    ...
    {
        oc1(k)= std::log(errors_list_l2(k)/errors_list_l2(k+1))/std::log(2);//Nd(k+1)/Nd(k));
    }
    std::cout<< "L2 order of convergence: " << oc1 << std::endl ;

    for (unsigned int k=0; k<MaxCycle-1; ++k)
    {
        oc1(k)= std::log(errors_list_h2(k)/errors_list_h2(k+1))/std::log(2);//Nd(k+1)/Nd(k));
    }
    std::cout<< "H2 order of convergence: " << oc1 << std::endl ;

    for (unsigned int k=0; k<MaxCycle-1; ++k)
    {
        oc1(k)= std::log(errors_list(k)/errors_list(k+1))/std::log(2);//Nd(k+1)/Nd(k));
    }
